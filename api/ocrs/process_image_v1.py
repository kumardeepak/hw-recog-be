...
def extract_horizontal_lines(filepath, debug=False):
...
def extract_vertical_lines(filepath, debug=False):
    org_img, process_img, hori_img, vert_img = extract_horizontal_vertical_line_images(filepath)
...
def extract_tables(filepath, output_dir):
    ...
    for index, contour in enumerate(contours):
        ...
        if (w*h > 200000) and (w*h < 800000):
            filename = os.path.join(output_dir, str(int(cont_ind/len(contours))) + "_" + str(int(cont_ind%len(contours))) + "_" + os.path.basename(filepath) + (".jpg" if (len(os.path.splitext(os.path.basename(input_filepath))) == 1) else ""))
...
def process_table_image(hori_filtered_lines, vert_filtered_lines, filepath, output_dir):
    rsp = {
        'header': {
            'title': '',
            'row': '',
    ...
    if len(hori_filtered_lines) == 12 and len(vert_filtered_lines) == 6:
        rsp['header']['title']      = 'Marks received'
        rsp['header']['row']        = 11
        rsp['header']['col']        = 6

    if len(hori_filtered_lines) == 8 and len(vert_filtered_lines) == 3:
    ...
    for row in range(len(hori_filtered_lines) - 1):
        for col in range(len(vert_filtered_lines) - 1):
            filename = os.path.join(output_dir, str(row) + "_" + str(col)+"_"+os.path.basename(filepath) + (".jpg" if (len(os.path.splitext(os.path.basename(input_filepath))) == 1) else ""))
